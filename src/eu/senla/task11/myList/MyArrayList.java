...
import eu.senla.task11.exceptions.*;
...
public class MyArrayList<DataType> implements MyList<DataType> {
    ...
    public MyArrayList (int capacity){
        ...
        // если capacity = 0, то инициализируем пустой массив
            ...
            throw new MyIllegalArgumentExcept("Ошибка аргумента, аргумент не может быть " +
                    "отрицательным числом, вы ввели: " + capacity);
        }
    }
    ...
    public MyArrayList(MyList<? extends DataType> collection){
        if (collection == null){
            throw new MyNullPointerExcept("Вы пытаетесь импортировать массив равный null");
        } else {
...
//        }
    ...
    private void checkIndexIfAdd(int index) {
        if (index < 0 || index > size)
            throw new MyArrayIndexOutOfBoundsExcept("Индекс: " + index + " не существует.");
    }
